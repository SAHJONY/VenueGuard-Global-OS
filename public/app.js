...
try {
  const [summary, events, ecosystem, catalog, platform, risk, supply, trace, integrations] = await Promise.all([json("/api/summary"), json("/api/events"), json("/api/ecosystem"), json("/api/catalog"), json("/api/platform"), json("/api/risk"), json("/api/supply"), json("/api/trace"), json("/api/integrations")]);
  ...
  document.querySelector("#readiness").innerHTML = Object.entries(integrations.integrations).map(([service, state]) => `<div><span>${service}</span><strong class="${state.configured ? "implemented" : "pending"}">${state.state} · ${state.missingCount} fields missing</strong></div>`).join("");
